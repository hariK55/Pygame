...
namespace AnythingWorld.Utilities
{
    public class JsonDeserializer 
    {
        public static List<ModelJson> DeserializeModelJsonList(string json)
        {
            ...
            {
                jsonReader.MaxDepth = 256; // increase allowed depth
                JArray modelJsonArray = JArray.Load(jsonReader);
        ...
        public static ModelJson DeserializeModelJson(string json)
        {
            ...
            {
                jsonReader.MaxDepth = 256; // increase allowed depth
                JObject modelJsonObject = JObject.Load(jsonReader);
                ...
                if (modelJsonObject.ContainsKey("model"))
                {
                    ...
                    {
                        if (((JObject)modelJsonObject["textures"]).TryGetValue("original", out JToken textures))
                            processedModelJson.model.other.texture = JsonConvert.DeserializeObject<string[]>(textures.ToString()).Distinct().ToArray();
                    }
                }
            }
